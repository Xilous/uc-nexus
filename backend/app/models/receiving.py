...
class ReceiveRecord(Base):
    ...
    line_items: Mapped[list["ReceiveLineItem"]] = relationship(
        back_populates="receive_record"
    )
...
class ReceiveLineItem(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    receive_record_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("receive_records.id"), nullable=False
    )
