...
def upgrade() -> None:
    ...
    op.create_table(
        'receive_line_items',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column('receive_record_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('receive_records.id'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('po_line_item_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('po_line_items.id'), nullable=False),
        sa.Column('receive_line_item_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('receive_line_items.id'), nullable=False),
        sa.Column('hardware_category', sa.String(), nullable=False),
        sa.Column('product_code', sa.String(), nullable=False),
